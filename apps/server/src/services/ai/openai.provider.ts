...
가사 언어 규칙 (최우선 규칙 — 장르와 무관하게 반드시 준수):
- 먼저 일기 7개의 내용을 전체적으로 분석하여 어떤 언어로 주로 작성되었는지 파악하세요.
- 일기가 전부 한국어면 → 가사를 반드시 한국어로 작성. 장르가 hip-hop, rock, R&B 등 영어권 장르여도 가사는 반드시 한국어로 작성할 것.
...
- 중요: 장르 선택은 음악 스타일에만 영향을 주고, 가사 언어와는 무관합니다. 일기 언어가 가사 언어를 결정합니다.
- lyricalTheme과 keywords도 일기 내용의 주요 언어에 맞춰 작성하세요.
...
Lyrics language rules (HIGHEST PRIORITY — must follow regardless of genre):
- First, analyze all 7 diary entries to determine the primary language(s) used.
- If all diaries are in Korean → lyrics MUST be written entirely in Korean. Even if the genre is hip-hop, rock, R&B, or any English-associated genre, the lyrics MUST be in Korean.
- If all diaries are in English → write lyrics entirely in English
...
- Important: Genre selection only affects the musical style, NOT the lyrics language. The diary language determines the lyrics language.
- lyricalTheme and keywords should also match the primary language of the diary content.
