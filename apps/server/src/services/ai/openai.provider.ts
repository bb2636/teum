...
다음 JSON 형식으로 정확히 응답해주세요 (JSON만 출력, 다른 텍스트 없이):
{
  ...
  "lyrics": "완성된 가사 (한국어, 정확히 5문단, 각 문단 정확히 3줄, 문단 사이 빈 줄로 구분). 반드시 일기 원문을 그대로 인용하지 말고, 일기에서 느껴지는 감정·상황·분위기를 바탕으로 완전히 새롭게 시적이고 음악적으로 재구성할 것. 원문의 문장 구조나 표현을 직접적으로 옮기지 말고, 감정의 핵심만 추출하여 노래 가사에 맞게 창작할 것. 은유, 비유, 감성적 표현을 풍부하게 사용할 것. 중요: 일기에 부정적이거나 우울한 내용이 있더라도 가사는 반드시 희망적이고 위로가 되는 방향으로 승화시킬 것. 슬픔을 인정하되 극복과 치유의 메시지를 담을 것. 쉼표(,) 사용 규칙: 줄 끝에 쉼표를 절대 넣지 말 것. 쉼표는 한 줄 안에서 나열이나 대등절을 연결할 때만 사용하고, 줄의 마지막 단어 뒤에는 쉼표 없이 끝낼 것. 예시 — 좋은 예: '어둠이 깊었던 날 마음속에 피어나는 작은 불빛', 나쁜 예: '어둠이 깊었던 날, 마음속에 피어나는 작은 불빛,'",
